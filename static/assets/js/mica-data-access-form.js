...
angular.module('formModule', ['schemaForm', 'hc.marked', 'angularMoment', 'schemaForm-datepicker', 'schemaForm-timepicker', 'schemaForm-datetimepicker', 'sfObibaSelectionTree', 'sfObibaFileUpload', 'ngFileUpload', 'ui.bootstrap'])
  ...
  .controller('FormController', ['$scope', '$filter', '$translate', '$http', function ($scope, $filter, $translate, $http) {
    $scope.forms = {};

    formModel.institution = '';
    ...
    $http.get(MicaService.normalizeUrl(`/ws/data-access-request/${darId}/model`), {
      ...
      if (response.data && response.data.name) {
        $scope.model.institution = response.data.name;
        if (response.data.staff) {
          $scope.model.staff = response.data.staff.map(function(item, index) {
            return angular.extend({}, item, {
              remove: $scope.model.staff && $scope.model.staff[index]
                      ? $scope.model.staff[index].remove
                      : false
            });
          });
        }
